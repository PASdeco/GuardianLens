  ...
  return (
    <div className="public-report-page">
      ...
      <main className="public-report">
        ...
        <div className="report-columns"><section><span className="eyebrow">Assessment fields</span>{[["Recall",assessment.recall_status],["Claims",assessment.claims_status],["Authority",assessment.authority_status],["Sponsorship",assessment.sponsorship_status],["Seller",assessment.seller_status]].map(([label,value]) => <div className="report-field" key={label}><span>{label}</span><strong>{value.replaceAll("_", " ").toLowerCase()}</strong></div>)}</section><section><span className="eyebrow">Supporting sources</span>{assessment.source_ids.map((source) => <div className="source-row" key={source}><span><FileCheck2 /></span><div><strong>{source}</strong><small>Validator-reviewed public source</small></div><ExternalLink /></div>)}</section></div>
